...
public class NotificationService {
    ...
    public void prepareEventAndSend(BirthRegistrationRequest request){
        ...
        request.getBirthRegistrationApplications().forEach(application -> {
            SMSRequest smsRequest = SMSRequest.builder().mobileNumber(application.getApplicantMobileNumber()).message(getCustomMessage(smsTemplate, application)).build();
    ...
    private String getCustomMessage(String template, BirthRegistrationApplication application) {
        ...
        template = template.replace("{NAME}", application.getFatherName());
        return template;
    }
